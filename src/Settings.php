...
final class Settings
{
    ...
    public static function register(): void
    {
        ...
        add_action('admin_post_newss_clear_all_jobs', [self::class, 'handleClearAllJobs']);
    ...
    /**
     * Loescht ALLE Newss-AS-Actions (pending + in-progress + failed + complete)
     * inkl. zugehoerige Logs, Locks und Pending-Transients.
     * Kanaele bleiben, Posts bleiben. Nur die Queue wird geleert.
     */
    public static function handleClearAllJobs(): void
    {
        if (!current_user_can('manage_options')) {
            wp_die('Forbidden');
        }
        check_admin_referer('newss_clear_all_jobs');
        ...
        // Video-IDs sammeln fuer Lock+Transient-Cleanup
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT action_id, args FROM {$actionsTable} WHERE hook = %s",
            Worker::HOOK_PROCESS
        ), ARRAY_A) ?: [];
        $count = count($rows);
        $videoIds = [];
        foreach ($rows as $row) {
            $args = json_decode((string) $row['args'], true);
            if (is_array($args) && isset($args[0]['video_id'])) {
                $videoIds[] = (string) $args[0]['video_id'];
            }
        }

        if ($count > 0) {
            // 1) Logs zuerst -- Foreign-Key auf action_id
            $wpdb->query($wpdb->prepare(
                "DELETE logs FROM {$logsTable} logs
                 INNER JOIN {$actionsTable} a ON a.action_id = logs.action_id
                 WHERE a.hook = %s",
                Worker::HOOK_PROCESS
            ));
            // 2) Actions
            $wpdb->query($wpdb->prepare(
                "DELETE FROM {$actionsTable} WHERE hook = %s",
                Worker::HOOK_PROCESS
            ));
            // 3) Orphan-Claims aufraeumen (Claims deren letzte Action jetzt weg ist)
            $wpdb->query(
                "DELETE c FROM {$claimsTable} c
                 LEFT JOIN {$actionsTable} a ON a.claim_id = c.claim_id
        ...
        foreach (array_unique($videoIds) as $vid) {
            if ($vid !== '') {
                delete_option('newss_lock_' . $vid);
                delete_transient(Worker::pendingTransientKey($vid));
            }
        }

        // Poll-Mutex + Progress-Option ebenfalls resetten
        ...
        // Auto-Cleanup-Rate-Limit resetten (falls User gleich wieder was queued)
        delete_transient('newss_auto_cleanup_ran');

        $user = wp_get_current_user()->user_login ?? '?';
        Logger::warn(sprintf('clear-all: %d Newss-actions + logs + locks deleted by %s', $count, $user));

        set_transient('newss_pipeline_notice', [
            'type'    => 'success',
            'message' => sprintf(
                '%d Newss-Jobs komplett gelöscht (alle Statuses) inkl. Logs und Locks. Kanäle und Posts sind unverändert. Beim nächsten Channel-Poll (oder manuellem „Jetzt manuell pollen") werden neue Jobs enqueued.',
                $count
            ),
        ], 60);
